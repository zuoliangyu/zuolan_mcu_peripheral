...
SSD1309是一個單晶片CMOS OLED/PLED驅動器，帶有用於有機/聚合物發光的控制器二極體點陣圖形顯示系統。 它由128個段和64個公共區組成。 這個IC是為普通陰極型OLED面板設計。 SSD1309內寘對比度控制、顯示RAM和振盪器，减少了外部組件和功耗。 它有256階亮度控制。 數據/命令通過硬體可選的6800/8000平行介面、I2C介面或串列週邊介面發送。 它適用於許多小型可擕式應用，如手機分屏、MP3播放機、小算盘等。

LibDriver SSD1309是LibDriver推出的SSD1309的全功能驅動，該驅動提供繪製單點，填充塊，圖片顯示等功能並且它符合MISRA標準。
...
/src目錄包含了LibDriver SSD1309的源文件。

/interface目錄包含了LibDriver SSD1309與平台無關的IIC、SPI總線模板。

/test目錄包含了LibDriver SSD1309驅動測試程序，該程序可以簡單的測試芯片必要功能。

/example目錄包含了LibDriver SSD1309編程範例。

/doc目錄包含了LibDriver SSD1309離線文檔。

/datasheet目錄包含了SSD1309數據手冊。
...
在線文檔: [https://www.libdriver.com/docs/ssd1309/index.html](https://www.libdriver.com/docs/ssd1309/index.html)。
