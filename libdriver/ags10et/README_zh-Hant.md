...
AGS10ET是一款採用數位信號輸出的MEMS TVOC感測器。 配寘了專用的數位模塊採集科技和氣體感應傳感技術，確保了產品具有極高的可靠性與卓越的長期穩定性，同時具有低功耗、高靈敏度、快速回應、成本低、驅動電路簡單等特點。

LibDriver AGS10ET是LibDriver推出的AGS10ET全功能驅動，該驅動提供TVOC讀取等功能並且它符合MISRA標準。
...
/src目錄包含了LibDriver AGS10ET的源文件。

/interface目錄包含了LibDriver AGS10ET與平台無關的IIC總線模板。

/test目錄包含了LibDriver AGS10ET驅動測試程序，該程序可以簡單的測試芯片必要功能。

/example目錄包含了LibDriver AGS10ET編程範例。

/doc目錄包含了LibDriver AGS10ET離線文檔。

/datasheet目錄包含了AGS10ET數據手冊。
...
在線文檔: [https://www.libdriver.com/docs/ags10et/index.html](https://www.libdriver.com/docs/ags10et/index.html)。
